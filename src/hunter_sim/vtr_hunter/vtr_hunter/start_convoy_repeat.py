...
repeat_msg.type = MissionCommand.ADD_GOAL                                      # CHANGE
repeat_msg.goal_handle = GoalHandle()
repeat_msg.goal_handle.type = GoalHandle.REPEAT
repeat_msg.goal_handle.waypoints = [50]
...
class ConvoyRepeatNode(Node):
    def __init__(self):
        ...
        self.command_pub_leader = self.create_publisher(MissionCommand, '/vtr/mission_command', 10)     # CHANGE
        self.command_pub_follower = self.create_publisher(MissionCommand, '/vtr_follower/mission_command', 10)     # CHANGE
        timer_period = 1.0
